# 2d-renderer Specification

## Purpose

Defines `engine_render`'s 2D rendering surface: a `Renderer` type that wraps SDL's GPU-accelerated 2D pipeline, presents at a fixed logical resolution scaled to the window, and exposes a small set of drawing primitives (clear, sprite draw, present). It also defines `Camera2D`, the 2D view transform that translates world-space draws into logical-screen space. The renderer is constructed and owned by `engine_runtime`; game code accesses it through `onStart`/`onRender` callbacks.
