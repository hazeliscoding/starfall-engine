...
Defines `engine_render`'s frame-based sprite animation capability: an `AnimationClip` data shape carrying an ordered list of `TextureHandle`s plus timing/looping metadata, and an `AnimatedSprite` that owns a set of named clips, advances frames against a wall-clock delta passed to `Update(dt)`, and exposes the current frame as a `TextureHandle` for the renderer to draw. This is the foundation for walk cycles, idle bobs, and other per-entity frame animation; it deliberately stays at the "named clips + Play/Pause/Update" level and does not yet cover skeletal, blended, or event-driven animation.
