...
Defines `engine_audio`, the Starfall Engine module that owns audio output for the runtime. It wraps the chosen audio backend (default device), exposes a small typed surface for streaming music (one track at a time, with fades and loop points) and polyphonic one-shot SFX (fixed-channel pool), and offers master/music/SFX volume controls. The `AudioSystem` is constructed and destroyed by `engine_runtime`, is the only path through which the game produces sound, and is designed to degrade gracefully (silent no-ops, error logs) when the backend cannot open a device — so the game still runs on headless CI and audio-less environments.
