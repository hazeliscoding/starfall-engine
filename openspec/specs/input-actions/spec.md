...
Defines `engine_input`'s action-mapping surface and the frame-coherent input-state contract that game code (C++ now, Lua later) uses to query player intent. This capability lets game code ask "is `MoveUp` held?" instead of "is the `W` scancode pressed?", decoupling gameplay from physical key bindings and enabling per-frame, edge-triggered semantics that survive rebinding and future input sources (gamepad, touch).
